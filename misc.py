...
def compareGraphs(g1: GaussianGraph, g2: GaussianGraph):
...
def isModelCorrect(model, g):
    g1 = g
    g2 = getGaussianGraph(model.G)
    correct, output = compareGraphs(g1, g2)
    if correct:
        print("Graphs are equivalent.")
    else:
        A, B, rk1, rk2 = output
        print(f"Different! Comparing {A} vs {B}..")
        print(f"Actual graph is rank {rk1} but model is rank {rk2}..")
